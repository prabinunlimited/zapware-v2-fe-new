...
import { faArrowRight, faTimes, faCheckCircle, faChevronDown, faChevronUp } from "@fortawesome/free-solid-svg-icons";
import { useNavigate } from "react-router-dom";
import ProgressBar from "../../../components/ProgressBar/ProgressBar";
import useCurrentStep from "../../../components/ProgressBar/useCurrentStep";

// Placeholder images - replace with your actual images
const userIcon = "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1000&q=80";
const institutionIcon = "https://images.unsplash.com/photo-1568992687947-868a62a9f521?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1000&q=80";
const partnerIcon = "https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1000&q=80";
  ...
  const handleSelectAccount = (type) => {
    ...
    setTimeout(() => {
      ...
      if (type === "partner") {
        ...
        navigate("/opencurrencyaccount", { state: { accountType: type } });
      }
    }, 300);
  };
  ...
  const accountTypes = [
    {
      ...
      description: "Sign up for a personalized account to access tailored services and features designed for personal use.",
      icon: userIcon,
      color: "blue",
      features: ["Personal dashboard", "Easy money transfers", "Bill payments", "Savings tools", "24/7 customer support"]
    },
    {
      id: "institution",
      title: "Institution Account",
      description: "Create an account for your organization to manage services efficiently with team collaboration features.",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 flex justify-center items-center py-8 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-6xl text-center relative z-10">
        ...
        <div className={`grid grid-cols-1 ${accountTypes.length > 2 ? 'md:grid-cols-3' : 'md:grid-cols-2'} gap-4 sm:gap-6 md:gap-8`}>
          {accountTypes.map((account) => (
            <div
              ...
              className={`relative bg-white rounded-xl sm:rounded-2xl shadow-lg overflow-hidden cursor-pointer transform transition-all duration-300 min-h-[380px] sm:min-h-[420px]
                ${selectedAccount === account.id ? 'ring-2 ring-offset-2 scale-105' : ''} 
                ${getBorderColor(account.color)} 
                ${tappedAccount === account.id ? 'scale-[1.02]' : ''}
                hover:shadow-xl`}
              onMouseEnter={() => !isTouchDevice() && setExpandedAccount(account.id)}
              onMouseLeave={() => !isTouchDevice() && setExpandedAccount(null)}
              onClick={() => isTouchDevice() ? handleAccountTap(account.id) : handleSelectAccount(account.id)}
            >
              {/* Image Section */}
              <div className="relative h-40 sm:h-48 overflow-hidden transition-all duration-500">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <h2 className="text-lg sm:text-xl font-bold text-white">{account.title}</h2>
                  <p className="text-white/90 text-xs sm:text-sm mt-1 line-clamp-2">{account.description}</p>
                </div>
                ...
                {isTouchDevice() && (
                  <div className="absolute top-3 left-3 w-6 h-6 rounded-full bg-black/40 flex items-center justify-center">
                    <FontAwesomeIcon
                      icon={expandedAccount === account.id ? faChevronUp : faChevronDown}
              ...
              <div className="p-4 sm:p-5">
                <div className="flex justify-between items-center mb-3 sm:mb-4">
                  <span className="text-xs sm:text-sm font-medium text-gray-500">Key Features</span>
                ...
                <button
                  ...
                  className={`w-full py-2 sm:py-3 px-4 ${getButtonColor(account.color)} text-white rounded-lg sm:rounded-xl transition-all duration-300 flex items-center justify-center font-medium text-sm sm:text-base shadow-md hover:shadow-lg`}
                >
              ...
              <div className={`absolute inset-0 bg-gradient-to-b ${getGradientColor(account.color)} to-black/95 p-4 sm:p-5 flex flex-col justify-center transition-all duration-500 ease-in-out 
                ${(expandedAccount === account.id || (!isTouchDevice() && expandedAccount === account.id)) ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>

                <h3 className="text-lg sm:text-xl font-bold text-white mb-3 sm:mb-4">Key Features</h3>
                <ul className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                  {account.features.map((feature, index) => (
                    <li key={index} className="flex items-start text-white">
                      <FontAwesomeIcon icon={faCheckCircle} className="text-white mr-2 mt-0.5 text-xs sm:text-sm bg-white/20 p-1 rounded-full flex-shrink-0" />
                ...
                <div className="mt-auto">
                  ...
                  >
                    <span className="mr-2">Select {account.title.split(' ')[0]}</span>
                    <FontAwesomeIcon icon={faArrowRight} className="text-xs sm:text-sm" />
                  </button>
                </div>
              </div>
