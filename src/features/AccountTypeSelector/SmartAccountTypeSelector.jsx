// src/features/Auth/components/SmartAccountTypeSelector.jsx
...
import { generatePasscode } from "../Auth/authThunk";

const SmartAccountTypeSelector = ({
  isOpen,
  onClose,
  accounts,
  email,
  ...
  const isGeneratingPasscode = useSelector(
    (state) => state.auth.isGeneratingPasscode,
  );
  ...
  useEffect(() => {
    if (accounts && accounts.length > 0 && !selectedType) {
      ...
      const cached = localStorage.getItem("last_account_selection");
      if (cached) {
        try {
          const {
            email: cachedEmail,
  ...
  try {
    ...
    // Retry passcode generation with selected type
    ...
    if (result.success) {
      ...
      onSuccess?.({
        ...result,
        customer_type: selectedType,  // Pass customer_type back
      });
      onClose();
    } else if (result.status === "multiple_accounts") {
      setError("Still having issues. Please contact support.");
    } else {
      setError(result.message || "Failed to generate passcode");
    }
  } catch (err) {
    setError(
      err.message ||
            ...
            <div className="p-6">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-600 mt-1">
                      We found multiple accounts associated with {email}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <label className="flex items-center justify-between p-3 bg-gray-50 rounded-lg mb-4 cursor-pointer">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    rememberChoice ? "bg-blue-600" : "bg-gray-300"
                  }`}
                >
                  <span
              ...
              <div className="flex space-x-3">
                ...
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-3 rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all disabled:opacity-50 flex items-center justify-center space-x-2"
                >
                  {isProcessing ? (
                    <>
              ...
              <p className="text-xs text-gray-500 text-center mt-4">
                ...
                <button
                  onClick={() => {
                    ...
                    localStorage.removeItem("last_account_selection");
...
export default SmartAccountTypeSelector;
