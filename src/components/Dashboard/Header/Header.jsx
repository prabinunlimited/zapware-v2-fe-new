...
const Header = ({ customerId }) => {
    ...
    const isWhitelabelledCustomerPartnerId = useSelector(selectIsWhitelabelledCustomerPartnerId);
    const fetchStatus = useSelector(selectFetchStatus);

    // Get from localStorage
    const isWhitelabelledCustomer = localStorage.getItem("isWhitelabelledCustomer") || "N";
    ...
    useEffect(() => {
        ...
        console.log("🔍 Current localStorage firstName:", localStorage.getItem("firstName"));
        console.log("🔍 profileLoading:", profileLoading);
        console.log("🔍 fetchStatus.profile:", fetchStatus.profile);
        console.log("🔍 profileData:", profileData);
        console.log("🔍 profileError:", profileError);
        console.log("🔍 =======================================");
    }, [authtoken, customerId, isStaffLogin, staffId, bearertoken, profileLoading, fetchStatus.profile, profileData, profileError]);
    ...
    useEffect(() => {
        if (bearertoken && fetchStatus.fx !== 'loading' && fetchStatus.fx !== 'succeeded') {
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.profile && data.profile.first_name) {
                        ...
                        console.log("🚨 Emergency fetch SUCCESS - Saved:", data.profile.first_name);
                    ...
                    console.log("🚨 Emergency fetch failed with status:", response.status);
                }
            } catch (error) {
        ...
        const timer = setTimeout(() => {
            if (!localStorage.getItem("firstName")) {
                console.log("🚨 No firstName found after 3s, triggering emergency fetch");
    ...
    const ProfileSection = useMemo(() => {
        ...
        const userRole = isStaffLogin === "1" ? staffRole : 
                        isOwnerLogin === "1" ? ownerRoleName : "Customer";
    ...
    const logoText = useMemo(() => {
        ...
        return "Dashboard";
    }, [isRemittanceOnlyCustomer, isWhitelabelledCustomerPartnerId, partnerConfig]);
                            ...
                            {partnerConfigLoading ? (
                                ...
                                <>
                                    ...
                                    <div className="flex flex-col">
                                        <span className="font-bold text-2xl tracking-tight">{logoText}</span>
                                        <span className="text-sm opacity-90 font-medium mt-1">Customer ID: {customerId}</span>
                                    </div>
                                </>
                            )}
                        </Link>
