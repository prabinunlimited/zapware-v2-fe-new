...
const SuccessModal = ({ response, onClose }) => {
      ...
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full">
        <div className="flex flex-col gap-4">
              ...
              <div className="mt-4 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                ...
                <ul className="text-sm text-yellow-700 space-y-2">
                  {response.failed_accounts.map((account, index) => (
                    <li key={index} className="flex items-start">
                      <svg
                        className="w-5 h-5 mr-2 text-yellow-600 mt-0.5 flex-shrink-0"
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path
          ...
          <div className="flex justify-center mt-6">
            <button
              onClick={onClose}
              className="px-6 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors min-w-[120px] font-medium"
            >
              OK
            </button>
          </div>
