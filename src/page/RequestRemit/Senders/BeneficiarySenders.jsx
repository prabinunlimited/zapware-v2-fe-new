// src/features/BeneficiarySenders/components/BeneficiarySenders.js
import React, {
  useState,
  useEffect,
  useCallback,
...
const API_URL =
  import.meta.env.REACT_APP_API_URL || "https://zapware.unlimitedremit.com/api";
    ...
    <div className="mt-6 p-4 bg-gray-100 rounded-lg border border-gray-300">
      <div className="flex items-center justify-between mb-2">
        <h3 className="font-bold text-gray-800 flex items-center">
          <FaBug className="mr-2" /> Debug Info
        </h3>
        <div className="flex gap-2">
          <button
          ...
          <button
            onClick={() => {
              localStorage.removeItem(`tempSenders_${benefId}`);
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm mb-3">
        <div>
          ...
          <div className="font-mono bg-white p-1 rounded">
            {benefId || "None"}
          </div>
        </div>
        <div>
          <strong>Total Senders:</strong>
          <div className="font-mono bg-white p-1 rounded">
      ...
      {showDetails && (
        <div className="space-y-3">
          ...
          {beneficiaries.length > 0 && (
            <div>
              <strong className="text-sm">Senders Found:</strong>
              <pre className="text-xs bg-gray-800 text-white p-2 rounded mt-1 overflow-auto max-h-40">
...
const BeneficiarySenders = () => {
  ...
  const { benefId: paramBenefId } = useParams();
  ...
  // Get the correct beneficiary ID
  const benefId = useMemo(() => {
    return (
      paramBenefId ||
  ...
  console.log("🔍 Component mounted with benefId:", benefId);
  ...
  // Data fetching - FIXED VERSION (removed the problematic filter)
  const fetchBeneficiaries = useCallback(async () => {
    ...
    try {
      console.log("🔍 Calling API: /beneficiaries/senders/", benefId);
      ...
      if (
        ...
        console.log(
          "✅ Found senders data:",
          result.getbenefsendersacctobeneficiaryid_data.length,
          "senders"
        );

        // CRITICAL FIX: REMOVED THE FILTER - API already returns correct data
        const sendersData = result.getbenefsendersacctobeneficiaryid_data;

        // Log each sender for debugging
        sendersData.forEach((sender, index) => {
          console.log(`🔍 Sender ${index}:`, {
            id: sender.id,
            beneficiary_id: sender.beneficiary_id,
            customer_id: sender.customer_id,
            customer_name: `${sender.customer?.first_name || ""} ${
              sender.customer?.last_name || ""
            }`.trim(),
            matches_current_beneficiary: sender.beneficiary_id == benefId,
          });
        });

        const sendersWithVisibility = sendersData.map((sender) => ({
          id: sender.id,
          // Sender (customer) information
          name: `${sender.customer?.first_name || ""} ${
            sender.customer?.middle_name || ""
          } ${sender.customer?.last_name || ""}`.trim(),
          email: sender.customer?.email || "No email",
          full_phone_number: sender.customer?.full_mobile_number || "No phone",
          street:
            sender.customer?.street_address_1 ||
            sender.customer?.street_address_2 ||
            "Not Available",
          // Store the actual customer_id from the sender
          customer_id: sender.customer_id,
          // Beneficiary information (who they sent to)
          beneficiary_id: sender.beneficiary_id,
          relationtobenef: "Sender",
          isVisible: true,
          // Add debug info
          debug: {
            original_beneficiary_id: sender.beneficiary_id,
            current_benefId: benefId,
            matches: sender.beneficiary_id == benefId,
          },
        }));

        console.log("✅ Mapped senders:", sendersWithVisibility);
        setBeneficiaries(sendersWithVisibility);
      } else {
        console.log(
          "❌ No senders data received or unexpected structure. Full response:",
          result
        );
      ...
      console.log(
        "✅ Fetch completed. Current beneficiaries:",
        beneficiaries.length
      );
    }
  }, [benefId, apiCall]);
  ...
  useEffect(() => {
    ...
    if (token && benefId) {
      console.log("✅ Starting to fetch beneficiaries...");
      fetchBeneficiaries();
    } else {
      console.log("❌ Cannot fetch - missing:", {
        token: !token ? "token" : "",
        benefId: !benefId ? "benefId" : "",
      });
    }
  ...
  const handleAddSender = async (customer) => {
    ...
    try {
      ...
      console.log("✅ Add sender response:", result);
      ...
      // Check if it's a specific error message we can handle
      if (
        error.message.includes("already exists") ||
        error.message.includes("duplicate")
      ) {
        toast.warning("This sender is already associated with the beneficiary");
  ...
  const handleSave = useCallback(
    async (customerIdFromSender, beneficiaryIdFromSender) => {
      const senderCustomerId = customerIdFromSender;
      const beneficiaryId = benefId;
      ...
      setSaveLoadingId(senderCustomerId);
      ...
      try {
        ...
        if (
          ...
          toast.warning(
            "This sender is already associated with the beneficiary"
          );
        } else {
  ...
  // Navigation
  const navigation = {
    editBeneficiary: (benefId) => navigate(`/editbeneficiary/${benefId}`),
    handleRoute: () => navigate(`/addbeneficiary/${customerId}`),
    handleBankRoute: () => navigate(`/addbeneficiarybank/${customerId}`),
    handleBeneficiaryBank: (benefId) =>
      navigate(`/bankdetails/${customerId}/${benefId}`),
    handleAddBeneficiary: () => navigate(`/addbeneficiary/${customerId}`),
    goBack: () => navigate(-1),
  };
  ...
  const handleAddTestData = () => {
    const testSender = {
      ...
      full_phone_number: "+1 234-567-8900",
      street: "123 Test St",
      customer_id: 99999,
      beneficiary_id: parseInt(benefId) || 25824,
      relationtobenef: "Sender",
      isVisible: true,
      isTest: true,
    };

    setBeneficiaries((prev) => [...prev, testSender]);
    toast.success("Test sender added!");
  };
          ...
          <div className="space-y-3">
            ...
            <div className="text-sm text-gray-600 space-y-1">
              ...
              {sender.debug && (
                <p className="text-xs text-gray-400 mt-2">
                  ID: {sender.customer_id} | Beneficiary:{" "}
                  {sender.beneficiary_id}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              <button
                ...
                disabled={saveLoadingId === sender.customer_id || sender.isTest}
                className="flex-1 flex items-center justify-center px-3 py-2 bg-teal-600 text-white rounded-md text-sm hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                ...
                {saveLoadingId === sender.customer_id ? "Saving..." : "Save"}
              </button>
            </div>
          </div>
        </motion.div>
                ...
                <td className="py-4 px-6 whitespace-nowrap w-1/4">
                  <div className="flex items-center justify-center space-x-2">
                    ...
                    <motion.button
                      ...
                      disabled={
                        saveLoadingId === sender.customer_id || sender.isTest
                      }
  ...
  const renderEmptyState = () => {
    if (hasSearched || searchQuery.trim() || filterVisibility !== "all") {
      return (
        <motion.div
        ...
        >
          ...
          <h3 className="text-2xl font-semibold text-gray-800 mb-3">
            No senders match your search or filter
          </h3>
          <p className="text-gray-600 max-w-md mb-6">
            Try adjusting your search criteria or filters.
          </p>
          {(searchQuery.trim() || filterVisibility !== "all") && (
            <motion.button
              onClick={handleClearSearch}
              className="mt-3 bg-gray-200 text-gray-800 py-2.5 px-6 rounded-lg shadow-md flex items-center space-x-2 hover:bg-gray-300 transition-all"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.97 }}
            >
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <motion.button
            ...
            className="bg-yellow-500 text-white py-2.5 px-6 rounded-lg shadow-md flex items-center space-x-2 hover:bg-yellow-600 transition-all"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.97 }}
          >
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-50 to-white py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <button
                  ...
                  className="text-xs bg-gray-200 text-gray-800 px-2 py-1 rounded flex items-center gap-1"
                >
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            <motion.button
              ...
              className="text-blue-600 border border-blue-600 py-2.5 px-5 rounded-lg shadow-lg flex items-center space-x-2 hover:bg-blue-50 transition-colors w-full sm:w-auto justify-center"
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex rounded-lg shadow-sm">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="block w-full pl-10 pr-10 py-3 border border-gray-300 rounded-l-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none transition-colors"
                    disabled={loading}
                  />
                  {searchQuery && (
                    <button
                ...
                <button
                  className={`py-3 px-6 rounded-r-lg font-medium transition-all duration-200 flex items-center justify-center border border-l-0 ${
                    headerColorProps.className || "bg-blue-600"
                  } text-white border-transparent hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed`}
                  style={headerColorProps?.style}
              ...
              <p className="text-xs text-gray-500 mt-2">
                {searchLoading
                  ? "Searching..."
                  : "Type to search automatically (min 3 characters required)"}
              </p>
            </div>
            ...
            {(searchQuery || filterVisibility !== "all") && (
              <div className="w-full md:w-auto flex-shrink-0">
                <button
                  ...
                  className="w-full md:w-auto px-4 py-3 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500"
                  disabled={loading}
                >
                  Clear All
                </button>
        ...
        {loading && !hasSearched ? (
          ...
          <FullPageLoader show={true} message="Searching senders..." />
