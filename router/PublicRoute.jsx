...
const PublicRoute = () => {
  ...
  if (hasCustomerIdButNoToken) {
    console.log(
      "🔄 [PublicRoute] Clearing inconsistent auth state: customerId exists but no token"
    );
  ...
  console.log("🔍 [PublicRoute] Route check:", {
    ...
    isAlwaysPublic,
    matchingRoutes: alwaysPublicRoutes.filter((route) =>
      location.pathname.includes(route)
    ),
  });
  ...
  console.log(
    "✅ [PublicRoute] User is not authenticated, allowing public access"
  );
  return <Outlet />;
};
